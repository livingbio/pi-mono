...
function createRunner(sandboxConfig: SandboxConfig, channelId: string, channelDir: string): AgentRunner {
	...
	// Create tools (each runner gets its own attach tool instance with isolated uploadFn)
	...
	if (loadedSession.messages.length > 0) {
		agent.state.messages = sanitizeOversizedImages(loadedSession.messages);
	...
	return {
		async run(
			...
			if (reloadedSession.messages.length > 0) {
				agent.state.messages = sanitizeOversizedImages(reloadedSession.messages);
